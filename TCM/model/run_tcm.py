...
MODEL_CLASSES = {
    'bert': (BertConfig, BertForTCMClassification, BertTokenizer),
    'xlnet': (XLNetConfig, XLNetForSequenceClassification, XLNetTokenizer),
    'xlm': (XLMConfig, XLMForSequenceClassification, XLMTokenizer),
    'roberta': (RobertaConfig, RobertaForTCMclassification, RobertaTokenizer),
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    # }
    result = metrics.classification_report(out_label_ids.tolist(), preds.tolist(), output_dict=True)
    ...
    with open(output_eval_file, "w") as writer:
        ...
        logger.info("  %s = %s\n", "macro avg", str(result["macro avg"]))
...
def load_and_cache_examples(args, tokenizer, evaluate=False,test=False):
    ...
    if evaluate:
        ...
        cached_features_file = os.path.join(args.data_dir, 'cached_{}_{}_{}_{}'.format(
            ...
            str(args.max_seq_length),
            str(args.differentiation_element)
        )) # cached_train_ZY-BERT_128_full
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        logger.info("Loading features from cached file %s", cached_features_file)
        features = torch.load(cached_features_file)
    else:
        logger.info("Creating features from dataset file at %s", args.data_dir)
        label_list = tcm_processor.get_labels()
        ...
        features = convert_examples_to_features(examples,
                                                ...
                                                # has_knowledge_as_instances= args.has_knowledge_as_instances,
                                                # shuffle=args.do_shuffle
                                                )
        if args.local_rank in [-1, 0]:
