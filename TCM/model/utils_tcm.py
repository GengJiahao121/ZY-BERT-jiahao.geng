...
class TcmProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir,args=None):
        ...
        return self._create_examples(self._read_json(os.path.join(data_dir, "train.json")),self._get_kb(data_dir),args.no_knowledge)


    def get_dev_examples(self, data_dir,args=None):
        """See base class."""
        logger.info("LOOKING AT {} dev".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "dev.json")),self._get_kb(data_dir),args.no_knowledge)

    def get_test_examples(self, data_dir, args=None):
        """See base class."""
        logger.info("LOOKING AT {} dev".format(data_dir))
        return self._create_examples(self._read_json(os.path.join(data_dir, "test.json")),self._get_kb(data_dir),args.no_knowledge)
    ...
    def get_labels(self):
        ...
        with open("/root/autodl-tmp/ZY-BERT-main/TCM/data_preprocess/syndrome_vocab.txt", 'r',encoding='utf-8') as f:
            for line in f:
